...
from workers.task_queue import task_queue, QUEUE_NAME, RESULT_PREFIX

logger = logging.getLogger("voice_service.workers.worker_pool")

class KokoroWorkerPool:
    """Phase 9: Pre-Warmed Worker Pool Manager."""

    def __init__(self, num_workers: int = 2):
        self.num_workers = num_workers
        self.engine = KokoroEngine()
        self.is_running = False
        logger.info(f"Initialized Worker Pool with {num_workers} pre-warmed Kokoro workers")

    def process_single_job(self, task_data: dict) -> dict:
        """
        Executes synthesis for a single enqueued task job.
        Pre-warmed ONNX engine guarantees <20ms inference execution.
        """
        ...
        # Step 2: Query Cache (Redis RAM -> SSD Disk -> Supabase)
        wav_bytes, hit_source = server_cache.get_audio(cache_key)

        # Step 3: Synthesize if cache miss
        if not wav_bytes:
            wav_bytes, duration_sec = self.engine.generate_audio(text, voice, speed)
            server_cache.save_audio(cache_key, wav_bytes, text, voice, speed)
            source = "SYNTHESIZED"
        else:
            duration_sec = len(wav_bytes) / (24000 * 2)
            source = f"CACHE ({hit_source})"

        latency_ms = int((time.time() - start_time) * 1000)

        # Encode audio to base64 for JSON result payload
        ...
        # Update task result in Redis with 600s TTL
