...
export const GENERATOR_TEMPLATES: Record<string, Record<'beginner' | 'intermediate' | 'advanced', Module[]>> = {
  java_sde: {
    beginner: [
      {
        id: 'java_b_mod1',
...
export default function CareerBuilderClient() {
  ...
  const [selectedPath, setSelectedPath] = useState('java_sde');
  ...
  const [editWeeks, setEditWeeks] = useState<number>(2);
  const [editDiff, setEditDiff] = useState<'Beginner' | 'Intermediate' | 'Advanced'>('Beginner');

  // Embedded Playground State
  const [activePlaygroundQuest, setActivePlaygroundQuest] = useState<Quest | null>(null);
  ...
  const generateFallbackRoadmap = () => {
    const pathTemplates = GENERATOR_TEMPLATES[selectedPath] || GENERATOR_TEMPLATES.java_sde;
    ...
    let generatedModules = JSON.parse(JSON.stringify(template)) as Module[];
    ...
    generatedModules = generatedModules.map(m => ({
      ...
      quests: m.quests.map((q: any) => {
        let category: 'learning' | 'exam' | 'assignment' = 'assignment';
  ...
  const handleGenerateRoadmap = async () => {
    ...
    try {
      const skillTags = (user as any)?.skill_tags || [];
      const weakAreas = (user as any)?.weak_areas || [];
      ...
      const targetRole = (user as any)?.target_role || targetRoleName;

      const res = await api.post<{ ok: boolean; modules: Module[] }>('/api/career-builder/generate', {
        targetRole,
        skillTags,
        weakAreas,
        experienceLevel
  ...
  const startEditingModule = (m: Module) => {
  ...
  const javaToJsTranspiler = (javaCode: string) => {
    let js = javaCode;
  ...
  const verifySolution = () => {
    if (!activePlaygroundQuest) return;
    setOutput(null);
    try {
  ...
  const getQuestStatus = (qId: string, questsList: Quest[]) => {
    if (completedQuests.includes(qId)) return 'completed';
    const idx = questsList.findIndex(q => q.id === qId);
    if (idx === 0) return 'unlocked';
    const prev = questsList[idx - 1];
    if (completedQuests.includes(prev.id)) return 'unlocked';
    return 'locked';
  };
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', paddingBottom: 60 }} className="animate-fade-in">
      ...
      {!resumeGenerated ? (
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '80px 20px', textAlign: 'center' }} className="animate-fade-in">
          <div style={{
              ...
              <div style={{ position: 'relative', paddingLeft: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {CAREERS.map(c => {
                  ...
                  return (
                    <div
                      key={c.id}
                      onClick={() => !generating && setSelectedPath(c.id)}
                      className={`glass-card ${active ? 'card-glow-hover' : 'card-hover'}`}
                      style={{
                        padding: 16,
            ...
            <div className="glass-card" style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <div className="form-group" style={{ marginBottom: 4 }}>
                ...
                <input
                  ...
                  value={knownConcepts}
                  onChange={e => setKnownConcepts(e.target.value)}
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                            {mod.quests.map((q, qIdx) => {
                              ...
                              return (
                                <div
                                  key={q.id}
                                  onClick={() => status !== 'locked' && selectPlaygroundQuest(q)}
                                  className={`glass-card ${status === 'locked' ? '' : 'card-hover'}`}
                                  style={{
