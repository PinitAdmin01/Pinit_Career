...
// ── Types ─────────────────────────────────────────────────────────────────────
interface Student {
  id: string; display_name: string; username: string; register_number?: string;
...
interface ExamResult {
  ...
  exam_name: string; score: number; total_marks: number; percentage: number;
  badge_level: string; tab_switches: number; status: string; created_at: string;
}
interface AttendanceLog {
  id: string; user_id: string; display_name?: string;
  date: string; status: 'present' | 'absent' | 'late'; method?: string;
}
interface ClassStat {
  label: string; value: string | number; color: string; icon: string;
}

type Tab = 'classes' | 'progress' | 'exams' | 'attendance';
...
function StatCard({ label, value, color, icon }: ClassStat) {
  return (
...
export default function TeacherPage() {
  ...
  const [attend,  setAttend]  = useState<AttendanceLog[]>([]);
  const [loading, setLoading] = useState(true);
  const [search,  setSearch]  = useState('');
  const [sortBy,  setSortBy]  = useState<'ats_score'|'trust_score'|'career_dna_score'|'mission_streak'>('ats_score');
  const [sortDir, setSortDir] = useState<'asc'|'desc'>('desc');

  useEffect(() => {
    if (!user) return;
    if (!['teacher','admin'].includes(user.role)) { router.push('/dashboard'); return; }
    loadAll();
  ...
  async function loadExams() {
    try {
      const d = await api.get<{ results: ExamResult[] }>('/api/exam/results?limit=200');
      setExams(d.results || []);
    } catch { setExams([]); }
  }
  ...
  async function loadAttendance() {
    try {
      const d = await api.get<{ logs: AttendanceLog[] }>('/api/attendance/logs?limit=500');
      setAttend(d.logs || []);
    } catch { setAttend([]); }
  }
  ...
  const SortArrow = ({ col }: { col: typeof sortBy }) =>
    sortBy === col ? <span style={{ color:'var(--accent)' }}>{sortDir === 'desc' ? ' ↓' : ' ↑'}</span> : null;

  // ── Attendance calendar helpers ───────────────────────────────────────────
  const attendByDate: Record<string, { present:number; absent:number; late:number }> = {};
  attend.forEach(a => {
    const d = a.date?.slice(0, 10) || '';
    if (!attendByDate[d]) attendByDate[d] = { present:0, absent:0, late:0 };
    attendByDate[d][a.status as 'present'|'absent'|'late'] = (attendByDate[d][a.status as 'present'|'absent'|'late'] || 0) + 1;
  });
  ...
  return (
    <div style={{ maxWidth:1280, margin:'0 auto' }} className="animate-fade-in">
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(120px,1fr))', gap:10, marginBottom:20 }}>
        ...
        ] as ClassStat[]).map(s => <StatCard key={s.label} {...s} />)}
      </div>
            ...
            {loading ? <Skeleton h={80} /> : (
              exams.length === 0
                ...
                : (() => {
                    ...
                    exams.forEach(e => {
                      if (!byExam[e.exam_name]) byExam[e.exam_name] = [];
      ...
      {tab === 'progress' && (
        <>
          <div style={{ display:'flex', gap:8, marginBottom:14, flexWrap:'wrap' }}>
            <input placeholder="Search student / reg no." value={search}
              onChange={e => setSearch(e.target.value)}
              className="form-input" style={{ flex:1, minWidth:180, maxWidth:300 }} />
          </div>
          ...
          {loading ? (
            ...
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                ...
                <tbody>
                  {sorted.length === 0 ? (
                    <tr><td colSpan={8} style={{ textAlign:'center', padding:32, color:'var(--t3)' }}>No students found</td></tr>
                  ) : sorted.map((s, i) => {
          ...
          <div style={{ ...card, marginBottom:16 }}>
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
              {last14Days.map(date => {
                ...
                return (
                  <div key={date} title={`${date}: ${a.present}P / ${a.absent}A / ${a.late}L`}
