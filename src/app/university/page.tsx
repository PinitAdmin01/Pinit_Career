...
interface PlacementStats {
  total_students: number; placement_ready: number; ats_qualified: number;
  avg_ats: number; avg_trust: number; avg_dna: number; engaged_students: number;
}
interface TopStudent {
  id: string; display_name: string; register_number?: string;
  ats_score: number; trust_score: number; career_dna_score: number; recruiter_visibility: number;
}
interface DeptStat {
  dept_code: string; student_count: number; avg_ats: number; avg_trust: number;
}
interface EmployabilityReport {
  highly_employable: number; employable: number; needs_development: number;
  high_trust: number; highly_engaged: number; certified: number;
}
interface SkillGap {
...
export default function UniversityPage() {
  ...
  const [topStudents, setTop]  = useState<TopStudent[]>([]);
  const [deptStats,   setDept] = useState<DeptStat[]>([]);
  const [employ,  setEmploy]   = useState<EmployabilityReport | null>(null);
