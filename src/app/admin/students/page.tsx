...
interface StudentRow {
  ...
  career_dna_score: number | null;
  mission_streak:   number | null;
}

type SortKey = 'name' | 'ats' | 'dna' | 'trust' | 'streak' | 'created' | 'active';
...
function AdminStudentsContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const d = await api.get<{ users: StudentRow[]; total: number }>(`/api/admin/users?${p}`);
        if (cancelled) return;
        setRows(d.users || []);
        setTotal(d.total || 0);
      } catch (e: unknown) {
  ...
  const filtered = useMemo(() => {
    let r = rows;
    if (minAts)   r = r.filter(x => (x.ats_score        ?? 0) >= Number(minAts));
    if (minTrust) r = r.filter(x => (x.trust_score      ?? 0) >= Number(minTrust));
    if (minDna)   r = r.filter(x => (x.career_dna_score ?? 0) >= Number(minDna));

    const sorter: Record<SortKey, (a: StudentRow, b: StudentRow) => number> = {
  ...
  async function changeRole(id: string, role: string) {
    if (!confirm(`Change role to "${role}"? This will remove the user from the student roster.`)) return;
    try {
    ...
    <div style={{ maxWidth: 1280, margin: '0 auto' }} className="animate-fade-in">
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr', gap: 10 }}>
          <input
            placeholder="Search by name, username, register number..."
            value={search}
      ...
      <div style={{
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
            ...
            <tbody>
              {loading && (
                <tr><td colSpan={9} style={{ padding: 36, textAlign: 'center', color: 'var(--t3)' }}>Loading roster…</td></tr>
              )}
              {!loading && filtered.length === 0 && (
                <tr><td colSpan={9} style={{ padding: 36, textAlign: 'center', color: 'var(--t3)' }}>
              ...
              {!loading && filtered.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                    <button onClick={() => changeRole(r.id, 'teacher')}
                            title="Promote to teacher"
                            style={iconBtnStyle}>↑</button>
                    <button onClick={() => banUser(r.id, r.display_name)}
                            title="Ban user"
                            style={{ ...iconBtnStyle, color: 'var(--coral)' }}>✕</button>
                  </td>
                </tr>
              ))}
            </tbody>
...
function Th({
  label, col, sortKey, sortDir, onClick, align = 'center',
}: {
  label: string;
  col: SortKey | null;
  sortKey: SortKey;
