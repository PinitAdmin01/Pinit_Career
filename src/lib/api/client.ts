...
Ensure you return ONLY the JSON object. Do not include markdown code block formatting (like \`\`\`json).`;
  ...
  if (cleanPath === '/api/finance/pay-due') {
    if (typeof window !== 'undefined') {
      ...
      // Append to transaction log for admin — use authenticated profile identity
      const profile = uid ? await fs.getUserProfile(uid) as any : null;
      let transactions = JSON.parse(localStorage.getItem('finance_transactions') || '[]');
      transactions.unshift({
        id: transactionId,
        studentName: profile?.displayName || profile?.display_name || profile?.username || 'Student',
  ...
  if (cleanPath === '/api/services/book-counselling') {
    if (typeof window !== 'undefined') {
      ...
      const newSession = {
        ...
        studentName: (await getActorIdentity()).name,
  ...
  if (cleanPath === '/api/research/publish-paper') {
    if (typeof window !== 'undefined') {
      ...
      const newPaper = {
        ...
        authors: authors || (await getActorIdentity()).name,
