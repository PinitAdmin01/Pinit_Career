...
// ── Main component ────────────────────────────────────────────────────────────
export default function PinITExamEngine({ exam, studentId, onFinish }: Props) {
  ...
  const { display: timeDisplay, urgent } = useCountdown(
  ...
  function scoreExam() {
    ...
    exam.questions.forEach(q => {
      if (q.type === 'mcq') {
        ...
        if (r) score += Math.round(q.marks * (r.passed / Math.max(r.total, 1)));
      }
  ...
  async function handleSubmit() {
    ...
    try {
      await api.post('/api/exam/sync-result', {
        examId:      exam.id,
        score,
  ...
  if (submitted) {
    ...
    return (
      <div style={{ maxWidth:500, margin:'60px auto', textAlign:'center', padding:32, background:'var(--bg2)', borderRadius:'var(--radius-xl)', border:'1px solid var(--border)' }}>
        ...
        {tabSwitches > maxSwitches && (
          <div style={{ marginTop:12, padding:'8px 14px', background:'var(--amber-light)', borderRadius:8, fontSize:12, color:'var(--amber)', fontWeight:600 }}>
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'220px 1fr', minHeight:'80vh', background:'var(--bg)', gap:0 }}>
      {/* ── Question navigator ── */}
      <div style={{ background:'var(--bg2)', borderRight:'1px solid var(--border)', padding:16, display:'flex', flexDirection:'column', gap:8 }}>
        ...
        {exam.questions.map((ques, i) => {
          ...
          return (
            <button key={ques.id} onClick={() => setCurrentQ(i)} style={{
      ...
      <div style={{ padding:28, overflowY:'auto' }}>
        {tabWarning && (
          <div style={{ marginBottom:16, padding:'12px 16px', background:'var(--coral-light)', border:'1px solid var(--coral)', borderRadius:'var(--radius)', fontSize:13, fontWeight:700, color:'var(--coral)' }}>
        ...
        {q.type === 'essay' && (
          <textarea
            ...
            style={{ width:'100%', padding:'12px 16px', borderRadius:'var(--radius)', border:'1px solid var(--border)', background:'var(--bg3)', color:'var(--t1)', fontSize:14, fontFamily:'inherit', lineHeight:1.65, resize:'vertical' }}
          />
        )}
        ...
        {q.type === 'coding' && (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <textarea
              ...
              rows={14}
              spellCheck={false}
              style={{ width:'100%', padding:'12px 16px', borderRadius:'var(--radius)', border:'1px solid var(--border)', background:'#0d1117', color:'#e6edf3', fontSize:13, fontFamily:'var(--font-mono)', lineHeight:1.6, resize:'vertical', tabSize:4 }}
              onKeyDown={e => {
